...
use super::{MAX_PLAINTEXT_BYTES, SHARED_PREVIEW_SCOPE_PREFIX};
use crate::error::{Error, Result};

pub(super) const MAX_RANGE_BYTES: u64 = 6 * 1024 * 1024;

pub(super) struct CacheFileGuard {
    path: Option<PathBuf>,
}
...
pub(super) struct SharedUploadCacheFileGuard {
    path: Option<PathBuf>,
}
...
pub(super) fn valid_cache_id(value: &str) -> bool {
    Uuid::parse_str(value)
        .is_ok_and(|uuid| uuid.to_string() == value && uuid.get_version() == Some(Version::Random))
}

pub(super) fn validate_range(start: u64, end: u64) -> Result<()> {
...
pub(super) fn ensure_file_size(path: &Path, expected_size: u64) -> Result<()> {
...
pub(super) fn file_matches(path: &Path, expected_size: u64, expected_sha256: &str) -> Result<bool> {
...
pub(super) async fn file_matches_async(
    path: PathBuf,
    expected_size: u64,
...
pub(super) async fn file_matches_cancellable_async(
    path: PathBuf,
    expected_size: u64,
    expected_sha256: String,
...
pub(super) fn file_matches_cancellable(
    path: &Path,
    expected_size: u64,
    expected_sha256: &str,
    cancellation: &tokio_util::sync::CancellationToken,
...
pub(super) fn snapshot_verified_file(
    source_path: &Path,
    cache_path: &Path,
    expected_size: u64,
    expected_sha256: &str,
...
pub(super) fn private_cache_path(root: &Path, cache_id: &str) -> Result<PathBuf> {
...
pub(super) async fn create_shared_upload_cache_root(path: &Path) -> Result<()> {
...
pub(super) fn shared_upload_cache_path(root: &Path, cache_id: &str) -> Result<PathBuf> {
    if !valid_cache_id(cache_id) {
        return Err(Error::InvalidTransferState);
    }
...
pub(super) fn cleanup_shared_upload_path(path: &Path) -> Result<bool> {
...
pub(super) fn clear_attachment_cache_directory(path: &Path) -> Result<()> {
...
pub(super) fn cached_shared_attachment_path(scope_path: &Path, cache_id: &str) -> PathBuf {
    scope_path.join(format!("{cache_id}.bin"))
}

pub(super) fn shared_cache_metadata_path(scope_path: &Path, cache_id: &str) -> PathBuf {
    scope_path.join(format!("{cache_id}.meta"))
}

pub(super) fn write_shared_cache_metadata(
    scope_path: &Path,
    cache_id: &str,
    size_bytes: u64,
    sha256: &str,
...
pub(super) fn commit_shared_cache_entry(
    scope_path: &Path,
    cache_id: &str,
    data_path: &Path,
    size_bytes: u64,
...
pub(super) fn read_shared_cache_metadata(
    scope_path: &Path,
    cache_id: &str,
...
pub(super) fn shared_cache_matches(
    scope_path: &Path,
    cache_id: &str,
    expected_size: u64,
    expected_sha256: &str,
...
pub(super) async fn shared_cache_matches_async(
    scope_path: PathBuf,
    cache_id: String,
    expected_size: u64,
    expected_sha256: String,
...
pub(super) fn shared_cache_id(scope_id: &str, attachment_id: &str) -> String {
    let mut hasher = Sha256::new();
...
#[cfg(unix)]
pub(super) fn sync_destination_directory(path: &Path) -> Result<()> {
    std::fs::File::open(path)?.sync_all()?;
    Ok(())
}

#[cfg(not(unix))]
pub(super) fn sync_destination_directory(_path: &Path) -> Result<()> {
    Ok(())
}
