...
fn run_denoise_blocking(
    ...
    let samples = anlg_audio_utils::resample_audio(source, DENOISE_SAMPLE_RATE)
        .map_err(|e| crate::Error::DenoiseError(e.to_string()))?;

    let channel_buffers = anlg_audio_utils::deinterleave(&samples, channels);
    ...
    let output = anlg_audio_utils::interleave(&output_channels);
